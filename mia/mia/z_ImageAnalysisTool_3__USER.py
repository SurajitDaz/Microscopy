...
def call_AnalyzeCellData():
    ...
    command = ["python", "z_AnalyzeCellData.py", image_path, selected_groups_str]
    try:
        # Run the external script
        subprocess.run(command, check=True)
        #print(f"Successfully initialized image processing with file: {excel_file}")
...
def call_Auto_cell_profile():
    global image_path
    command = ["python", "z_Auto_cell_profile.py", image_path]
    try:
        # Run the external script
        subprocess.run(command, check=True)
        #print(f"Successfully initialized image processing with file: {excel_file}")
...
def open_slider_window(option):
    ...
    if option == "HIP Perceived Grayscale Analysis":
    ...
    elif option == "HIP Actual Grascale Analysis":
        slider1 = tk.Scale(slider_window, from_=0, to=255, resolution=0.01, orient=tk.HORIZONTAL, label="Shift Gray", variable=slider1_var)
        slider1.pack(fill=tk.X, padx=5, pady=5)
    ...
    elif option == "HIP Color based Analysis":
        slider1 = tk.Scale(slider_window, from_=0, to=1, resolution=0.01, orient=tk.HORIZONTAL, label="Red Balance", variable=slider1_var)
        slider1.pack(fill=tk.X, padx=5, pady=5)

        slider2 = tk.Scale(slider_window, from_=0, to=1, resolution=0.01, orient=tk.HORIZONTAL, label="Green Balance", variable=slider2_var)
        slider2.pack(fill=tk.X, padx=5, pady=5)

        slider3 = tk.Scale(slider_window, from_=0, to=1, resolution=0.01, orient=tk.HORIZONTAL, label="Blue Balance", variable=slider3_var)
        slider3.pack(fill=tk.X, padx=5, pady=5)

    elif option == "Analogy of HDCS":
        slider1 = tk.Scale(slider_window, from_=0, to=1, resolution=0.01, orient=tk.HORIZONTAL, label="Hue", variable=slider1_var)
        slider1.pack(fill=tk.X, padx=5, pady=5)

        slider2 = tk.Scale(slider_window, from_=0, to=1, resolution=0.01, orient=tk.HORIZONTAL, label="Saturation", variable=slider2_var)
        slider2.pack(fill=tk.X, padx=5, pady=5)

        slider3 = tk.Scale(slider_window, from_=0, to=1, resolution=0.01, orient=tk.HORIZONTAL, label="Contrast", variable=slider3_var)
        slider3.pack(fill=tk.X, padx=5, pady=5)
    '''    
...
def reset_sliders(slider_window, option):
    ...
    """
    if option == "HIP Perceived Grayscale Analysis":
        slider1_var.set(110)  # Default value for slider 1
    ...
    elif option == "HIP Actual Grayscale Analysis":
        ...
        slider1_var.set(0)  # Default value for slider 1
        slider2_var.set(0)  # Default value for slider 2
        slider3_var.set(0)  # Default value for slider 3
    elif option == "Analogy of HDCS":
        slider1_var.set(0)  # Default value for slider 1
        slider2_var.set(0)  # Default value for slider 2
...
def on_dropdown_select(*args):
    ...
    if selected_option in ["HIP Perceived Grayscale Analysis", "HIP Actual Grascale Analysis", "HIP RGB VE ", "Analogy of HDCS"]: 
...
DEFAULT_IMAGE_PATH = "mia/util/img/pic2.png"
...
color_spaces = ["HIP Perceived Grayscale Analysis", "HIP Actual Grascale Analysis", "HIP RGB Analogy", 
                "Differential Chroma Shift", "X-Clustering", "Deeplearning (UNET-VGG19)", "Deeplearning(PSPNet)", "Deeplearning(Inception)"]
...
button_menu = tk.Menubutton(dropdown_frame, text="Denoising", indicatoron=True, borderwidth=1, direction="below")
button_menu.pack(side=tk.LEFT, padx=5, pady=5)
...
# Add submenus
blob_detection_menu = tk.Menu(menu, tearoff=0)
blob_detection_menu.add_command(label="Hough Circle Transform", command=lambda: hough(image_path, status_label) )

blob_detection_menu.add_command(label="Advanced Hough Circle Transform", command=lambda: Hough_Circle_Transform(image_path, status_label) )


blob_detection_menu.add_command(label="Connected Component Analysis", command=lambda: Connected_Component_Analysis(image_path, status_label))
menu.add_cascade(label="Blob Detection and Removal", menu=blob_detection_menu)

frequency_domain_menu = tk.Menu(menu, tearoff=0)
frequency_domain_menu.add_command(label="Fourier Transform & Band-Pass Filtering", command=lambda: ftbp(image_path, status_label))
frequency_domain_menu.add_command(label="Wavelet Transform", command=Wavelet_Transform)
menu.add_cascade(label="Frequency Domain Filtering", menu=frequency_domain_menu)

deep_learning_menu = tk.Menu(menu, tearoff=0)
deep_learning_menu.add_command(label="Denoising Autoencoders (DAE)", command=Denoising_Autoencoders)
deep_learning_menu.add_command(label="U-Net or GAN-based Restoration", command=Unet_GAN)
menu.add_cascade(label="Deep Learning-Based Approaches", menu=deep_learning_menu)

background_subtraction_menu = tk.Menu(menu, tearoff=0)
background_subtraction_menu.add_command(label="Gaussian or Median Filtering", command=Gausian_Median)
background_subtraction_menu.add_command(label="Rolling Ball Algorithm", command=lambda: Rolling_Ball(image_path, status_label))
menu.add_cascade(label="Background Subtraction", menu=background_subtraction_menu)

adaptive_filter_menu = tk.Menu(menu, tearoff=0)
adaptive_filter_menu.add_command(label="Bilateral Filtering", command=lambda: Bilat(image_path, status_label))
adaptive_filter_menu.add_command(label="Non-Local Means Filtering", command=lambda: NLMn(image_path, status_label))
adaptive_filter_menu.add_command(label="Median Filtering", command=lambda: Medfiltr(image_path, status_label))
menu.add_cascade(label="Adaptive Filter", menu=adaptive_filter_menu)
